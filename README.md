...
- The gate above narrows this class but does not eliminate it: an alphabetic-only value pushed into the location column by a malformed row, that no longer matches any patient hash, visit id, or provider id anywhere in the dataset, is indistinguishable from a real place name. It would still pass the automatic-weather gate and be geocoded if the user selects it in the location filter, because no count or collision check can recognize it as anything other than a place.
