...
- 148 unit tests on the data and weather logic, run in Node with no browser simulation
...
Patient and provider data are processed locally and are not inputs to either weather endpoint. The two Open-Meteo calls are the only outbound requests containing dashboard-derived values: a policy-approved place name, latitude and longitude, dates, variable names, and units. Before a geocoding URL can be built, a pure egress gate requires a text-only city and recognized U.S. state, uses own-key-only state recognition, and rejects a city query that matches any patient hash, visit ID, or provider ID across the full accepted dataset. Ambiguous and colliding values remain local. The one case the gate cannot recognize — an alphabetic-only shifted value that also ends in a recognized state suffix and no longer collides with any identifier — is documented under "Known limitations". Privacy regressions cover inherited-key suffixes, a grammar-valid identifier collision, and the exact compensating malformed row that shifts `patient-secret-900` into `location`; each proves that no geocoding URL is produced. A broader parsed-file test also asserts that no visit id, patient hash, or provider id appears in any URL on a non-vacuous allowed request path. Warning examples never include a patient hash. The lint configuration makes any `console` call a lint failure. No analytics, no storage, no cookies. The page also loads two font families from Google Fonts on first load; those resource requests carry no dashboard data, but they do expose the visitor's IP address and ordinary browser request metadata to Google. Self-hosting the fonts is a planned improvement.
...
- The gate above narrows this class but does not eliminate it: an alphabetic-only value pushed into the location column by a malformed row, that no longer matches any patient hash, visit id, or provider id anywhere in the dataset, is indistinguishable from a real place name. To reach the geocoder at all, that value must also end in a recognized U.S. state name or abbreviation; a bare token with no such suffix is gated as invalid regardless of collision status. But a shifted value that happens to end in a recognized state suffix and does not collide with any identifier would still pass the automatic-weather gate and be geocoded if the user selects it in the location filter, because no count or collision check can recognize it as anything other than a place.
